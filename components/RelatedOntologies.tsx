...
/**
 * Links a resource to the portal ontology entries it is *about* — a curated
 * relationship from the resource's `aboutOntologies` field. Each chip scrolls to
 * that ontology's card (/resources#<id>).
 */
    ...
    <div className="flex flex-wrap items-center gap-1.5 text-xs">
      ...
      {ontologies.map((o) => (
        <Link
          key={o.id}
          href={`/resources#${o.id}`}
          title={`About ${o.name} — view in the portal`}
        ...
        >
          {(o.name.match(/\(([^)]+)\)\s*$/)?.[1] ?? o.name).toUpperCase()}
        </Link>
      ))}
    </div>
  );
